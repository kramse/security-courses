...
Do the same with \verb+traceroute+

{\bf Solution:}\\
Run both programs to local gateway and some internet address by your own choice.

{\bf Discussion:}\\
Note the tool is called  \verb+tracert+ on Windows, shortened for some reason.

ICMP is the Internet Control Message Protocol, usually used for errors like host unreachable. The ECHO request ICMP message is the only ICMP message that generates a ICMP response!
...
You can also find web sites that will allow you to do a traceroute, like \link{http://traceroute.org/} -- not all sites on that work unfortunately.
...
{\bf Suggested method:}\\
Try one or more of the following programs:
\begin{itemize}
\item nslookup - Unix and Windows, but not recommended\\
Not performing a lookup exactly like the libraries, results may differ:\\
\verb+nslookup -q=txt -class=CHAOS version.bind. 0+
\item dig - syntax @server domain query-type query-class\\
The advanced tool for doing DNS work: \verb+dig @8.8.8.8 www.example.com+
\item host - syntaks host [-l] [-v] [-w] [-r] [-d] [-t querytype] [-a] host [server]\\
Easy to use and automate, my favourite: \verb+host www.example.com 8.8.8.8+
\end{itemize}
...
\begin{alltt}\small
$ host www.kramse.dk
www.kramse.dk has address 185.129.63.130
www.kramse.dk has IPv6 address 2a06:d380:0:102::80
...
\begin{list2}
\item Forward, from name to IP address -- uses typically A records, address records
\item Reverse, from IP address to name -- uses PTR records, pointer records
\end{list2}
...
{\bf Hints:}\\
Linux systems often have whois installed as a command line program, or can be installed quickly.

If you only rarely need to lookup, then another option is to use web sites for doing Whois lookups \link{https://apps.db.ripe.net/db-web-ui/\#/query} or their RIPEStat web site which can give even more information
...
The IP space is managed at the top by  Internet Assigned Numbers Authority (IANA),
\link{https://www.iana.org/}

\chapter{\faInfoCircle\ IP address research up to 30 min}
...
We can later talk more about \emph{IP reputation}
...
You can run Wireshark on your host too, if you want. Downloads available for Windows, Linux and Mac OS X
...
{\bf Suggested method:}\\
Download from \link{https://www.wireshark.org/} or your local package repository, then
run Wireshark (or tcpdump).


There are many tutorials for Wireshark - and it is a very advanced tool! The PPA book I recommend has on page 41: Your First Packet Capture.
...
Many network tools use the library pcap and data format pcap which is originally from the tool Tcpdump, see more at \link{https://www.tcpdump.org/}
...
\label{ex:nping-tcp}
{\bf Objective:} \\
Show the use of a more advanced ping tool named Nping for checking ports through a network
...
{\bf Suggested method:}\\
Install Nmap from the main site \link{https://nmap.org/}, or your local repository with \verb+apt install nmap+.

Run the command using a common port like Web HTTP 80/tcp:
...
{\bf Hints:} \\
A lot of options are similar between Nmap and Nping, and the tool is part of the Nmap package.
...
{\bf Objective:}\\
Use the network mapper Nmap to discover active systems
...
{\bf Hints:} \\
Try nmap in sweep mode - and you may run this from Kali or Debian
...
10.0.45.123/32, a whole subnet /24 about 250 hosts like 10.0.45.0/24 and other more advanced targeteting like 10.0.45.0/25 and 10.0.45.1-10
...
The main conclusion and security implications are, if you can connect to a service -- you can send data, including attacks. On the other hand, if a port is blocked neither and exploit, login with stolen credentials, a backdoor or similar can be sent! So by scanning and reducing your \emph{attack surface} your risk and exposure is reduced.
...
{\color{red} \bf This should only be done in closed networks or when there is an agreement to do this!}
...
{\bf Objective:}\\
Start an attack using TCP flooding tool hping3.
...
{\bf Objective:}\\
Start an attack using UDP flooding tool hping3.
...
{\bf Objective:}\\
Start an attack using ICMP flooding tool hping3.
...
{\bf Objective:}\\
Start an attack using the packet generator and flooding tool t50.
...
This demonstrates testing towards Opportunistic TLS ports for mail servers --
read more about this at: \url{https://en.wikipedia.org/wiki/Opportunistic_TLS}
...
Connected to 2a06:d380:0:102::25
...
...
Not valid before: Mar 22 22:01:47 2026 GMT
Not valid after:  Jun 20 22:01:46 2026 GMT

$ sslscan --starttls-smtp mail.kramse.org:587
Version: 2.1.5
OpenSSL 3.5.5 27 Jan 2026

Connected to 2a06:d380:0:102::25

Testing SSL server mail.kramse.org on port 587 using SNI name mail.kramse.org
...
{\bf Objective:}\\
Install a DNS server -- might be suitable for many environments.

{\bf This is an advanced exercises, and not one we will do in class!}
...
{\bf Objective:}\\
See and work with LibreNMS - for a little while. Ask for a demo!
