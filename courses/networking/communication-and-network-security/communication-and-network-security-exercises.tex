...
\chapter{Capturing TCP Session packets}
...
{\bfseries Opgave:} \\
L<E6>r at bruge whois

{\bfseries Forslag til fremgangsm<E5>de:}\\
\begin{list2}
\item Login p<E5> UNIX server - l<E6>s manualen til programmet whois
eller brug webinterface p<E5>\\ \link{http://www.ripe.net}
\end{list2}

{\bfseries Hj<E6>lp:}\\
Whois databaserne er fordelt p<E5> ARIN, RIPE, LACNIC og APNIC.

Kommandoen \verb+whois -r 90.184.69.97+ vil p<E5> en OpenBSD give
svaret p<E5> et opslag i RIPE databasen efter IP adresse 90.184.69.97

{\bfseries Diskussion:}\\
I skal l<E6>re at sp<F8>rge efter IP adresser og spore oprindelsen - find
...
{\bfseries Opgave:}\\
L<E6>r at bruge ping og traceroute programmerne

{\bfseries Forslag til fremgangsm<E5>de:} \\
Brug \verb+ping+ og \verb+traceroute+ til at teste netv<E6>rksforbindelsen - kan
udf<F8>res fra b<E5>de windows og UNIX.

Husk at traceroute hedder \verb+tracert+ p<E5> windows.

Er der forbindelse til alle servere p<E5> oversigtstegningen?
...
svar tilbage for at kunne mappe et netv<E6>rk.

{\bf Ekstra:}
Hvad er forskellen p<E5> (skal udf<F8>res p<E5> OpenBSD/UNIX)
\begin{list2}
\item {\bfseries traceroute} og {\bfseries traceroute -I}
\item NB: traceroute med -I findes kun p<E5> UNIX - traceroute med ICMP pakker
...
{\bfseries Opgave:}\\
Pr<F8>v forskellige programmer til at sp<F8>rge en service

{\bfseries Forslag til fremgangsm<E5>de:}\\
\begin{itemize}
\item nslookup - findes b<E5>de p<E5> UNIX og Windows
\item Pr<F8>v nslookup -q=txt -class=CHAOS version.bind. 0
\item dig - syntaks @server domain query-type query-class
\item host - syntaks host [-l] [-v] [-w] [-r] [-d] [-t querytype] [-a] host [server]
\item pr<F8>v {\bfseries host -a security6.net} \\
{\bfseries host -a www.security6.net} - hvad er forskellen
\end{itemize}

{\bfseries Hj<E6>lp:}\\
Host programmet er med som standard p<E5> OpenBSD - s<E5> brug Fiona eller Luffe

P<E5> Unix Boot CD og MS Windows platformen findes mange GUI programmer til det samme.
...
{\bfseries Opgave:}\\
log ind pï¿œ et OpenBSD UNIX system og se pï¿œ \verb+syslog.conf+
...
{\bfseries Hjï¿œlp:}\\
 Indholdet af filen kan ses pï¿œ nï¿œste slide
...
{\bfseries Diskussion:}\\
Se ogsï¿œ \link{http://www.loganalysis.org/}
...
{\bfseries Opgave:}\\
Log ind pï¿œ et OpenBSD UNIX system og se pï¿œ \verb+snmpwalk+
...
Public er default pï¿œ meget udstyr og findes stadig mange steder i produktion.

Private er et andet kendt community name og gav tidligere lov til at ï¿œndre indstillingerne pï¿œ enheder.
...
{\bfseries Opgave:}\\
Der skal laves e-mail server baseret pï¿œ postfix
...
portnummer - sï¿œ der ikke kommer konflikt med de andre pï¿œ holdet

NB: Hvis flere starter server der vil lytte pï¿œ port 25 giver det
...
Husk ogsï¿œ at oprette en fil \verb+.forward+ for brugere, sï¿œledes at
mail ikke strander pï¿œ servere. Denne fil skal blot indeholde en e-mail
...
\item Tilfï¿œj en regel for at tillade adgang til noget mere pï¿œ vores
  systemer
%\item Gï¿œ sammen i hold pr system
\end{list2}

Pï¿œ OpenBSD kan man se om firewall er enablet med kommandoen \verb+pfctl+. Prï¿œv fï¿œlgende kommandoer pï¿œ eksempelvis Luffe
\begin{list2}
...
{\bfseries Hjï¿œlp:}\\
Vi gennemgï¿œr sammen ï¿œndringen pï¿œ de forskellige firewalls vi har til
rï¿œdighed!

%Hint: se pï¿œ \verb+/etc/sysconfig/iptables+
...
Der er GUI pï¿œ Mac OS X, Windows, Linux og ogsï¿œ til OpenBSD PF pfw
