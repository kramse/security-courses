...
\chapter{Bonus: Use Ansible to install programs}
...
Due to python version 2 being deprecated there are some missing files for running the tool Zenmap. This can be fixed by manually downloading packages and messing with \verb+dpgk -i+ and \verb+apt+
\begin{enumerate}
...
\item Install pygtk in the same way, note: version number may have changed from this:\\
\verb+wget http://archive.ubuntu.com/ubuntu/pool/universe/p/pygtk/python-gtk2_2.24.0-5.1ubuntu2_amd64.deb+
\item I used \link{https://packages.ubuntu.com/xenial/amd64/python/python-gtk2} as a starting point, downloaded the library, tried installing, missing python-cairo and python-gobject, downloaded these and installed. YMMV.
\end{enumerate}
...
\link{https://www.kalilinux.in/2021/02/zenmap.html}
...
{\bf Purpose:}\\
Use a Filebeat which can take data in JSON and insert into Elasticsearch.
...
Note: your installation already has the Debian repositories, so just using \verb+apt install filebeat+ should be enough.
...
{\bf Hints:}\\
Filebeat also provides a Dashboard for Zeek, whichj can be inserted using:\\
...
{\bf Discussion:}\\
We will now spend a lot of time going into the Dashboard
...
Use Kali and the command line tools to test your server, if not available try ping - which will do a lookup. Retry the commands from exercise: {\bf \nameref{ex:basic-dns-lookup}}
...
{\bf Solution:}\\
When you have tried sending a few DNS requests you are done.

{\bf Discussion:}\\
How do we enforce new versions of protocol - as old as DNS?!

\link{https://dnsflagday.net}
