...
\begin{list1}
\item Der findes programmer som kan hjælpe med at udregne
...
\item Primære router conhome har IP-adressen 10.0.45.2 (og flere andre)
...
\item Det er tilladt at logge ind på alle systemer, undtagen Henrik's laptop :-)
...
\item -a viser alle
 -n viser kun adresserne numerisk
...
\vskip 1cm
\centerline{NDP og ARP har samme funktion, binder IP og MAC sammen}
...
\item Huskeliste til Henrik:
\begin{list2}
\item Airport Extreme access-point
\item Juniper SRX router/firewall
\item OpenBSD router
\item Linux Ubuntu config
\end{list2}
\end{list1}
...
\begin{alltt}
\emph{To netkort med fast IPv6 og DHCP på ydersiden}
...
\slide{Vigtigste protokoller - opsummering}
...
\begin{list1}
\item IP routing er nemt - {\bf longest match vinder altid}
...
\hlkimage{17cm}{ehandel-https.pdf}
...
\centerline{Husk også DANE DNS-Based Authentication of Named Entities RFC 6698}
...
\item BIND fra Internet Systems Consortium
\item konfigureres gennem \verb+named.conf+
\item det anbefales at bruge BIND version 9 eller senere
\end{list1}
...
\exercise{ex:test-ipv6}

\exercise{ex:test-ipv6-dns}
...
\item Der findes et antal værktøjer til test, blandt andet Nping og Iperf
\end{list1}
...
\centerline{God test af netværkslatency, samt TCP sockets på OS}
...
Ovenstående er set fra server, client kaldes med \verb+iperf -c 10.0.42.23+

NB: brug i praksis altid mindst 60 sekunder, \verb+iperf -i 5 -t 60 -c 10.0.42.23+
...
\vskip 1cm
\centerline{ab er godt til små test, i praksis er Tsung, Jmeter m.fl. bedre}
