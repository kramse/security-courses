...
They work, and will give a first impression of Zeek. If you later want specific features not configured into the binary packet, then install from source or the official packages from the Zeek project.

Also Zeek uses a zeekctl program to start/stop the tool, and a few config files which we should look at. From a Debian system they can often be found in \verb+/etc/zeek+ , but on our systems installed from their package we find it in \verb+/opt/zeek/etc+.

\begin{minted}[fontsize=\footnotesize]{shell}
root@debian:/opt/zeek/etc# ls -la
drwxr-xr-x   3 root root  4096 Oct  8 08:36 .
drwxr-xr-x 138 root root 12288 Oct  8 08:36 ..
-rw-r--r--   1 root root  2606 Oct 30  2020 zeekctl.cfg
-rw-r--r--   1 root root   225 Oct 30  2020 networks.cfg
-rw-r--r--   1 root root   644 Oct 30  2020 node.cfg
...
\begin{minted}[fontsize=\footnotesize]{shell}
root@debian:/etc/zeek# cat node.cfg
