...
\begin{enumerate}
\item Make sure OpenSSH server is installed on the Debian server: \verb+apt install openssh-server+
...
\chapter{\faExclamationTriangle\ CIS Benchmarks 30min}
\label{ex:CIS-benchmarks}
...
\chapter{\faExclamationTriangle\ Lynis Auditing, System hardening, and Compliance testing 30min}
...
# sudo nmap -A -p 22 --script "ssh2-enum-algos,ssh-auth-methods" 127.0.0.1
...
\chapter{\faExclamationTriangle\ Centralized syslog 30min}
...
{\bf Hints:}\\
Look in the config file, may be in /etc/rsyslog.conf, /etc/syslog  or /etc/syslog-ng/syslog-ng.conf
...
Main point is that syslog is a standard way of collecting and sending logs to a central place.
