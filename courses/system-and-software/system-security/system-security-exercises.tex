...
\begin{enumerate}
\item First download the {\bf Zenmap RPM} file with type \verb+.rpm+ from \url{https://nmap.org/download}
\item then convert to .deb file using the tool alien, \verb+alien *.rpm+
...
The use the utility tool \verb+ssh-copy-id+ for copying the public key to the server. Install tool if not available using \verb+apt+ :
\begin{alltt}
...
{\bf Hints:}\\
You can publish your SSH keys in public places such as Github and Ubuntu installation can fetch this during install, making the use of SSH keys extremely easy.
...
{\bf Suggested method:}\\
Write down the current status, What are the network parts, organization (team two people) and applications (Nginx running on single Debian server?).

Consider the impact of adding wireless network to this setup.
