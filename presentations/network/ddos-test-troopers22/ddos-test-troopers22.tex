...
{\Large TL;DR Not necessarily a single device}
...
\slide{Pressure Points and Scope}
...
\begin{list2}
\item Scope for me is everything that could adversely affect the network
\item Common scope IPv4: Link network /28 or /26 and a customer network /24
\item Common scope IPv6: Link network and a customer network /48
\end{list2}
...
My favourite tools have not supported IPV6, which is a shame.

Two options, modify tools or use newer tools

I truly love Hping3, this tools is very flexible and powerful,


\begin{list2}
\item Fortunately these tools are open source, and I modified Hping to suit my needs, search and replace for inet to inet6, so I could have an IPv6 DDoS testing tool\\
...
\slide{Performing a DDoS test }
BUT like Nmap and others, how do you perform some task? Listing options show multiple pages ...
...
# nmap -p 1-65535 -A -oA full-scan $CUST_NET4
