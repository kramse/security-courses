...
\item Benyttes stadig i visse sammenhænge
\end{list1}
\pause

\centerline{Idag vil vi ikke diskutere etik men blot snakke teknik}
...
\item BT4 er pt. i beta, og udleveres derfor ikke, få en kopi via HTTP lokalt eller via USB stick
...
\hlkimage{5cm}{pgp.png}

\begin{list1}
\item Lisbeth Salander bruger PGP, men er det sikkert?
\pause
\item Jeg bruger PGP \smiley
\pause
\item Politiet bruger det vist ikke liiiige, FTP er godt nok ...
\end{list1}
...
\hlkimage{15cm}{henrik-2009.jpg}

\begin{list1}
\item Visitkort - Henrik Lund Kramshøj, hlk@security6.net
\end{list1}
...
\slide{Live demo af alle de steder man finder data}

\vskip 2cm
\centerline{ingen grund til ligefrem at udlevere mig selv på skrift \smiley}

\begin{list2}
\item Husk: email header, webserver headers, google, google images, google groups, googledorks, tcpdump, wireshark, nmap, nikto, whois ripe, domæner, adresser, sslug billeder, msn, irc, ...
\end{list2}
...
\hlkimage{17cm}{htw2.png}

\begin{list1}
\item Onion routing - sender data gennem flere routere på en tilfældig måde per session
\item \link{http://www.torproject.org/}
\end{list1}
...
\begin{list1}
\item ... også en super måde at få passwords på \\
{\footnotesize \link{http://arstechnica.com/security/news/2007/09/security-expert-used-tor-to-collect-government-e-mail-passwords.ars}}
\item Han blev anholdt og fik en snak med politiet
\item Idag har det svenske politi selv lov til at sniffe ...
\end{list1}


\slide{Svenske politi}
...
Kilde: \link{http://stoppafralagen.nu/}

\vskip 1cm
\centerline{Tak, Sverige - nu er det ikke \emph{kun} paranoia, men fakta}
...
\slide{Beating TOR - NSA Geohacking}

\begin{quote}
Last week, the NSA was granted a patent which supposedly allows them to "Spot Network Snoops". At first glance, the patent seems rather obscure and boring.  However, it could have major implications for anonymity and privacy on the Internet.

Back in 2005, the same NSA inventor, Michael Reifer, and a colleague were granted a patent called "Method for Geolocating Logical Network Addresses" At the time, this made a bit of a splash. It was a technique for matching IP addresses to physical geographical locations, {\bf based solely on packet timing information.}
\end{quote}

Kilde:\\
{\footnotesize\link{http://philosecurity.org/2008/12/29/nsa-another-blow-against-internet-anonymity}}\\
...
\begin{quote}
Onsdag d. 4. juli (2003) vil Folketinget have vedtaget en ny lov, som giver politiet ret til at benytte civile agenter i deres efterforskning. Denne lov giver også mulighed for at {\bf hemmeligholde sagsakter for den tiltalte og hans forsvarer}. Den nye lov giver også myndighederne mulighed for at benytte {\bf nye tekniske metoder} til at skaffe sig oplysninger om den mistænkte ved at {\bf bryde ind i hans pc}. Med en hemmelig dommerkendelse. Det vil af denne vej være muligt - på løbende bånd - at ransage indholdet af dine kærestebreve, dagbøger og dine inderste tanker, som du har betroet apparatet. Selv om du kan se, at nogen roder i dine sager, ved du ikke, om det er politiet, en konkurrent eller bare din nysgerrige nabo. For at bryde ind i din pc {\bf vil politiet formentlig benytte en type af programmer}, som i fagkredse kaldes for en {\bf virus eller en trojansk hest}. Den trojanske hest vil løbende kunne sende data fra din pc ud til de skjulte personer, som sidder gemt i skyggerne og {\bf overvåger dig via internettet}.
\end{quote}
\begin{list1}
\item Vi kan jo ikke undgå lige at snakke om rockerloven \link{http://www.1984.dk}
\item Kilde: prosa.dk fra 2003, årstal 2003 og fed skrift tilføjet af mig i ovenstående
\end{list1}
